...
const Help = ({ updateDate = '', firstOpen = false, mode = '' }) => {
  ...
  return (
    <>
      <p>
        {mode === PARADOX_MODE && (
          <>{i18n.get('paradoxHelpDesc', { times: DEFAULT_TRY_TIMES }, true)}</>
        )}
        ...
        <hr />
        {firstOpen && i18n.get('helpFirstOpen') !== 'helpFirstOpen' && (
          <>
            {i18n.get('helpFirstOpen', null, true)}
            <hr />
          </>
        )}
        ...
        {updateDate && (
          <>
            <br />
            {i18n.get('helpUpdateTime', { updateDate: updateDate })}
          </>
        )}
        <br />
        {i18n.get('helpNum', { num: chartsData.length })}
      </p>
    </>
  );
};
