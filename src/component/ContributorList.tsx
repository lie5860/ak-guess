...
const ContributorItem = (props: { user: User }) => {
  ...
  return (
    <span
      className="mdui-list-item mdui-ripple user-item"
      onClick={() => {
        if (homeUrl) {
          window.open(homeUrl);
        }
...
const ContributorList = (props: IProps) => {
  ...
  return (
    <ul className="mdui-list user-list" key={icon}>
      ...
      {userList.map((user: User, index) => (
        <ContributorItem user={user} key={index} />
      ))}
      {[1, 2, 3, 4, 2, 2, 3, 4].map((a, i) => (
        <li key={i} className="mdui-list-placeholder  user-item" />
      ))}
    </ul>
  );
};
