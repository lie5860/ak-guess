...
export default function Home() {
  ...
  React.useEffect(() => {
    getDailyData(i18n.language).then(
      ({ last_date, daily }: { last_date: string; daily: number }) => {
  ...
  const menu = showDailyMode
    ? [RANDOM_MODE, DAILY_MODE, PARADOX_MODE]
    : [RANDOM_MODE, PARADOX_MODE];
  // 第一次进悖论模式，弹出帮助框
  React.useEffect(() => {
      ...
      <div className={`main-container clean-float lang-${i18n.language}`}>
        {config.showServer && (
          ...
          >
            ...
            <span
              className="mini-chip mdui-color-blue-a400 mdui-text-uppercase pointer font-mono"
              id="server-chip"
            >
        ...
        <ul id="server-menu" className="mdui-menu">
          <li className="mdui-menu-item mdui-ripple">
            {Object.keys(labelDict)
              .filter((v) => hostDict[v])
              .map((key) => {
                return (
                  <a
                    key={key}
                    className="mdui-ripple pointer"
                    onClick={() => {
                      location.href = `${location.protocol}//${location.host}${location.pathname}?lang=${key}`;
                    }}
                  >
                    <i
                      style={{ visibility: key === i18n.language ? undefined : 'hidden' }}
                      className="mdui-menu-item-icon mdui-icon material-icons"
                    >
                      done
                    </i>
                    {labelDict[key]}
          ...
          <div className="mdui-tab mdui-tab-scrollable ak-tab" mdui-tab>
            {menu.map((menuMode) => {
              return (
                <div
                  key={menuMode}
                  className={`ak-tab-item ${mode === menuMode ? 'active' : ''}`}
